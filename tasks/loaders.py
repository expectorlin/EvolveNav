...
def create_dataloaders(args, config, logger, training, device, feat_db=None, obj_feat_db=None, stage="finetune"):
    if training==False and stage=='pretrain':
        return None, None

    dataset_cfg = copy.deepcopy(config.Dataset)
    dataset_cfg.update(
        config.Pretrain if stage=="pretrain" else config.Finetune
    )
